...
### Template Analyst 模板分析师

- **定位**：连接人类优秀设计与机器代码的“拆解专家”。
- **职责**：全面解剖你上传的参考 PPT 模板，提取排版规律并将其翻译成下游机器能懂的指令。
- **核心工作**：
    - **切分聚类**：分析幻灯片的功能类型，分清哪些是支撑逻辑框架的“结构页”（如开场、目录），哪些是用于传递细节的“内容页”。
    - **提取 Schema 骨架**：将底层啰嗦的 XML 格式转化为清晰的数据骨架，梳理出每个页面具体有哪些坑位（文本、图片及其用途）。
    - **HTML 视图渲染**： 将幻灯片渲染成直观的 HTML 格式，为后续的排版步骤提供精准的导航地图。

### Research Team 资料研究团队

- **定位**：负责资料搜索与整合，将海量资料收敛成零散的知识。
- **职责**：吞吐你上传的参考资料（Given documents），并根据 Boss 下发的搜索主题向外拓展。
- **并行工作流**：内部包含三个并发执行的 Agent：
    - `SearchDocument`：挖掘深度文本资料。
    - `SearchImage`：搜集相关配图与视觉素材。
    - `SearchAnalytics`：抓取数据、图表支撑论点。
